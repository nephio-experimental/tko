...
Here's an example of an equivalent to `tko template get`:

    scripts/kubectl-kind get template/$(tko kube to demo/hello-world:v1.0.0) --output=yaml

Extract the YAML you want using [yq](https://github.com/mikefarah/yq):

    scripts/kubectl-kind get template/$(tko kube to demo/hello-world:v1.0.0) --output=yaml | yq '.spec.package.resources'

Getting just the package in JSON:

    scripts/kubectl-kind get template/$(tko kube to demo/hello-world:v1.0.0) --output=jsonpath='{.spec.package.resources}'
...
See the [KRM guide](KRM.md) for more information and examples.
